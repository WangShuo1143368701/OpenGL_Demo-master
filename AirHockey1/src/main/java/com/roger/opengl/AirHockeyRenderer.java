...
public class AirHockeyRenderer implements GLSurfaceView.Renderer {
  ...
  @Override public void onSurfaceCreated(GL10 gl10, EGLConfig eglConfig) {
    ...
    GLES20.glUseProgram(program);//告诉 OpenGL 在绘制任何东西在屏幕上的时候要使用这里定义的程序。//ShaderProgram
...
　　  第一条语句表示清除颜色设为黑色，第二条语句表示实际完成了把整个窗口清除为黑色的任务，glClear（）的唯一参数表示需要被清除的缓冲区。
    ...
    GLES20.glDrawArrays(GLES20.GL_TRIANGLES, 10, 6);//画三角形 一个顶点一个颜色
    ...
    /*glUniform4f 的作用是为当前程序对象指定Uniform变量的值。
    这里即设置了片段着色器中的 u_Color 的值，后四个参数分别为红色、绿色和蓝色还有透明度的值，
    一旦指定了颜色，接下来我们就可以调用 glDrawArrays 来绘制桌子了，第一个参数 GL_TRIANGLES 说明我们想绘制三角形，
    第二个参数表示告诉 OpenGL 从顶点数组的开头处开始读顶点，第三个参数是告诉 OpenGL 读入六个顶点。
    因为每个三角形有三个顶点，所以这里将绘制出两个三角形*/
  }
}
