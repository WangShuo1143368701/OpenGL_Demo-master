 ...
 ***/
package com.roger.heightmap.data;
...
public class VertexBuffer {
  ...
  public VertexBuffer(float[] vertexData) {
    ...
    glGenBuffers(buffers.length, buffers, 0);//创建一个缓冲区对象
    if (buffers[0] == 0) {
    ...
    // Bind to the buffer.
    glBindBuffer(GL_ARRAY_BUFFER, buffers[0]);//绑定 GL_ARRAY_BUFFER参数告诉openGL这是一个顶点缓冲区。

    // Transfer data to native memory.
    FloatBuffer vertexArray =
        ByteBuffer.allocateDirect(vertexData.length * Constands.BYTES_PER_FLOAT)
            .order(ByteOrder.nativeOrder())
            .asFloatBuffer()
            .put(vertexData);
    vertexArray.position(0);

    // Transfer data from native memory to the GPU buffer. 把数据传递到缓冲区对象
    glBufferData(GL_ARRAY_BUFFER, vertexArray.capacity() * Constands.BYTES_PER_FLOAT, vertexArray,
        GL_STATIC_DRAW);

    // IMPORTANT: Unbind from the buffer when we're done with it.
    glBindBuffer(GL_ARRAY_BUFFER, 0);//已经把数据传递到缓冲区对象了，就要解除绑定。
