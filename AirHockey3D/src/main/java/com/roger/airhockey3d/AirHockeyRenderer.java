...
public class AirHockeyRenderer implements GLSurfaceView.Renderer {
  ...
  private int program;
  float[] tableVertices = {
      //Order of coordinates:X,Y,R,G,B 去掉了Z w 以让其自动生成
  ...
  @Override public void onSurfaceChanged(GL10 gl10, int width, int height) {
    ...
    Matrix.setIdentityM(modelMatrix,0);//创建一个模型矩阵
    Matrix.translateM(modelMatrix,0,0f,0f,-2.5f);//对桌子在 z 轴上往负方向移动了2.5个单位。
    Matrix.rotateM(modelMatrix,0,-60f,1f,0f,0f);//把桌子绕着 x 轴旋转了 -60 度
    final float[] temp = new float[16];
    Matrix.multiplyMM(temp,0,projectionMatrix,0,modelMatrix,0);//透视投影矩阵与模型矩阵相乘 得出一个矩阵暂存在temp中
    System.arraycopy(temp,0,projectionMatrix,0,temp.length);//使用 System.arraycopy() 把结果存回 projectionMatrix 中
  ...
  @Override public void onDrawFrame(GL10 gl10) {
    ...
    GLES20.glUniformMatrix4fv(uMatrixLocation,1,false,projectionMatrix,0);//把定义的矩阵传递给着色器
