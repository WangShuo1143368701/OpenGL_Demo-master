...
public class AirHockeyRenderer implements GLSurfaceView.Renderer {
  ...
  @Override public void onSurfaceCreated(GL10 gl10, EGLConfig eglConfig) {
    ...
    uMatrixLocation = GLES20.glGetUniformLocation(program,U_MATRIX);//存储矩阵 uniform 的位置
  }

  @Override public void onSurfaceChanged(GL10 gl10, int width, int height) {
    GLES20.glViewport(0, 0, width,height);
/*
* 创建一个正交投影矩阵，这个矩阵会把屏幕的当前方向计算在内。我们首先计算了宽高比，
* 它使用宽高中的较大值除以较小值，所有不管横屏还是竖屏，这个值都是一样的。然后调用 orthoM() 方法。
* 在横屏模式下，我们会扩展宽度的坐标，以达到在横竖屏上都显示正常的效果。
* */
